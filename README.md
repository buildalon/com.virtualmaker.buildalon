...
A Build Pipeline utility package for the [Unity](https://unity.com/) Game Engine. This package provides extends Unity with command line arguments that are equivalent to the options in the Unity build window. You can use it to automate building Unity in any continuous integration and continuous deployment (CI/CD) environment. For the [Buildalon GitHub Actions](https://github.com/marketplace?query=buildalon) this package is designed to be use in conjunction with, this package adds logging and summary utilities.
